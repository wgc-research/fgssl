...
from federatedscope.gfl.dataset.PlanetoidForFgcl import PlanetoidForFgcl
import torch_geometric.transforms as T

INF = np.iinfo(np.int64).max
from federatedscope.gfl.dataloader.utils import process_client_data
...
def load_nodelevel_dataset(config=None):
    ...
    if name in ["cora", "citeseer", "pubmed"]:
        ...
        if config.data.fgcl is not None and config.data.fgcl == True:
            dataset = PlanetoidForFgcl(path,
                                       name,
                                       split='random',
        ...
        if config.data.fgcl is not None and config.data.fgcl == True:
            ...
            for client_data in dataset:
                ...
                dataset_client.append(client_data)
            dataset = dataset_client
        if config.data.fgcl is not None and config.data.fgcl == True:
            global_dataset = PlanetoidForFgcl(path,
                                              name,
                                              split='random',
                                              num_train_per_class=num_split[name][0],
                                              num_val=num_split[name][1],
