...
class visClient(Client):
    pass


class visServer(Server):
    def _perform_federated_aggregation(self):
        ...
        for model_idx in range(self.model_num):
            ...
            #
            #     visual_tsne(list_feat, self.state, labels, 6)
...
def call_my_worker(method):
    if method == 'vis':
        worker_builder = {'client': visClient, 'server': visServer}
        return worker_builder

register_worker('vis', call_my_worker)
...
def global_tsne(model,data,num_classes):
...
def visual_tsne(model, data,name):
    ...
    tsne = manifold.TSNE(n_components=2, perplexity=40, init='pca')
    ...
    sns.relplot(data=df_tsne, palette="bright", hue='class', x='dim1', y='dim2')
    plt.xticks([])
    plt.yticks([])
    import os
    if not os.path.exists('data/output/tsne/'):
        os.mkdir("data/output/tsne/")
    plt.savefig("data/output/tsne/result_"+ name + ".pdf", format='pdf', dpi=800,
...
def visual_cka(list_feat, state, labels, num_class, theme):
    ...
    plt.xlabel("ClientId")
    plt.ylabel("ClientId")
    plt.title("node-level")
    plt.savefig("data/output/cka/node_state_" + str(state) + ".png", dpi=500)
    plt.show()
    ...
    for i in range(client_number):
        for j in range(client_number):
            result[i][j] = CKA(list_feat[i]["x"].detach().numpy(), list_feat[j]["x"].detach().numpy())

    sns.heatmap(data=result, vmin=0.0, vmax=1.0, cmap=theme).invert_yaxis()
    plt.xlabel("ClientId")
    plt.ylabel("ClientId")
    plt.title("structure_level")
    plt.savefig("data/output/cka/structure_state_" + str(state) + ".png", dpi=500)
    plt.show()
